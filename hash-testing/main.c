...
char *normalizeStr(char *str, uint16_t *ln){
	uint16_t l = len(str);
...
int isInArr(uint64_t *arr, uint64_t hash){
	uint64_t i = 0;
	while(arr[i] != 0){
		if(arr[i] == hash)
			return 1;
...
int main(){
	...
	uint64_t i = 0, collisions = 0;
	uint64_t *arr = calloc(466551, sizeof(uint64_t));
